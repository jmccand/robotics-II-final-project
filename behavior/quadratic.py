import argparse

import matplotlib.pyplot as plt
import numpy as np

from behavior.behavior_controller import BehaviorController
from dynamics import Dynamics
from formation import Formation
from path import FunctionPath
from plot import Plot


def simulate(frames: int = 500, save: str | None = None):
    np.random.seed(42)

    # downward arc: y=0 at x=±4, y=-2 at x=0
    path = FunctionPath(
        f=lambda x: (x**2 - 16.0) / 8.0,
        df=lambda x: x / 4.0,
        x_start=-4.0,
        x_end=4.0,
    )
    formation = Formation.line(5, spacing=1.0)
    controller = BehaviorController()
    dynamics = Dynamics()
    plotter = Plot(path, formation)

    path_speed = 5.0
    t = 0.0

    tang0 = path.tangent(t)
    heading0 = np.arctan2(tang0[1], tang0[0])
    desired0 = formation.desired_positions(path.point(t), heading0)

    states = np.zeros((formation.n, 4))
    for i in range(formation.n):
        states[i, :2] = desired0[i] + np.random.randn(2) * 0.1

    for _ in range(frames):
        tang = path.tangent(t)
        heading = np.arctan2(tang[1], tang[0])
        desired = formation.desired_positions(path.point(t), heading)
        plotter.record(states, t, desired)

        new_states = np.zeros_like(states)
        for i in range(formation.n):
            v_cmd = controller.compute_control(i, states, path, t, formation, [])
            new_states[i] = dynamics.step(states[i], v_cmd)
        states = new_states
        t = min(t + path_speed * dynamics.dt, 1.0)

    plotter.animate()
    if save:
        plotter.save(save)
    else:
        plt.show()

    plotter.plot_deviations()
    plt.show()


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--save", default=None, help="Output filename (.mp4 or .gif)")
    parser.add_argument("--frames", type=int, default=500)
    args = parser.parse_args()
    simulate(frames=args.frames, save=args.save)
