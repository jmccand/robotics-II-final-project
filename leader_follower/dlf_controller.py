...
# robot 0 follows the path; every other robot follows robot 0 directly
class DirectLeaderFollowerController(FormationController):
    def __init__(
        self,
        k_leader: float = 8.0,
        k_tangent: float = 1.5,
    ...
    def _direct_slot(self, robot_idx: int, states: np.ndarray, formation: Formation, path: Path) -> np.ndarray:
        heading = self._leader_path_heading(states, path)
        c, s = np.cos(heading), np.sin(heading)
        R = np.array([[c, -s], [s, c]])
        rel_offset = formation.offsets[robot_idx] - formation.offsets[0]
        return states[0, :2] + R @ rel_offset
    ...
    def compute_control(
        ...
        if robot_idx == 0:
            ...
            slot = self._direct_slot(robot_idx, states, formation, path)
            f_follow = self.k_follow * (slot - pos)
            f_align = self.w_align * (states[0, 2:] - vel)
    ...
    def desired_positions(
        ...
        for i in range(1, formation.n):
            desired[i] = self._direct_slot(i, states, formation, path)
        return desired
